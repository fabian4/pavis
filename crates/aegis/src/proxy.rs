...
impl ProxyHttp for MyProxy {
    ...
    async fn request_filter(
        ...
        tracing::info!("Incoming request: method={}, path={}, host={:?}", req_header.method, uri_path, host_header);
    ...
    async fn upstream_peer(
        ...
        let upstream_name = match &ctx.upstream_name {
            Some(name) => name,
            None => return pingora::Error::e_explain(pingora::ErrorType::InternalError, "No upstream selected"),
        };

        let upstream = self.config.upstreams.iter().find(|u| &u.name == upstream_name);

        if let Some(u) = upstream {
            if u.endpoints.is_empty() {
                 return pingora::Error::e_explain(pingora::ErrorType::InternalError, "Upstream has no endpoints");
            }
            let mut rng = rand::rng();
        ...
        pingora::Error::e_explain(pingora::ErrorType::InternalError, "Upstream not found in config")
    }
