...
use pavis_e2e::support::upstream_tls_config;
...
async fn test_upstream_tls() {
    ...
    let config = upstream_tls_config(&listen_addr, &upstream_host, actual_upstream_port);
    write_config(&config_path, &config).expect("write upstream tls config");
