...
- Requests and assertions:
  - POST `/v1/publish` without version header -> 400.
  - POST `/v1/publish` with empty body -> 400.
  - POST `/v1/publish` with invalid payload -> 422.
  - POST `/v1/publish` with valid payload and version 1 -> 200.
  - GET `/v1/config` with version 0 -> 200 and:
    - `X-Pavis-Version: 1`
    - `X-Pavis-Checksum` present
    - `X-Pavis-Checksum-Alg` present
    - `Content-Type: application/octet-stream`
    - `Cache-Control: no-store`
    - body matches published bytes
...
- Requests and assertions:
  - GET `/v1/config?wait_ms=2000` with version 1 blocks.
  - POST `/v1/publish` with version 2 unblocks the wait.
  - Response is 200 with `X-Pavis-Version: 2` and body matches new bytes.
...
- Steps and assertions:
  ...
  - GET `/ready` -> 200.
  - GET `/v1/config` with version 1 -> 200 (no long-poll hold).
  - Response header `X-Pavis-Version` is `0` (fresh state).
  - Response body matches the persisted LKG bytes.
