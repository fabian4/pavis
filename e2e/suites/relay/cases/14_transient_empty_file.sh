...
# e2e/suites/relay/cases/14_transient_empty_file.sh
...
PORT_RELAY=8297
CASE_TMP=$(ensure_tmp_dir "relay_14")
...
# 5. Check version (should not have changed or could have changed depending on implementation)
v_after_empty=$(curl -s "http://127.0.0.1:$PORT_RELAY/v1/status" | grep -o '"version":[0-9]*' | cut -d: -f2)

# 6. Write valid config
...
# 7. Wait for version increment from last valid state
for i in {1..30}; do
    v_current=$(curl -s "http://127.0.0.1:$PORT_RELAY/v1/status" | grep -o '"version":[0-9]*' | cut -d: -f2)
    if [ "$v_current" -ge "$((v_after_empty + 1))" ]; then
        break
    fi
    sleep 0.2
done

# 8. Verify we recovered and incremented
v_final=$(curl -s "http://127.0.0.1:$PORT_RELAY/v1/status" | grep -o '"version":[0-9]*' | cut -d: -f2)
if [ "$v_final" -gt "$v_after_empty" ]; then
    echo "✅ Case 14_transient_empty_file passed"
else
    echo "❌ Expected version > $v_after_empty, got $v_final"
    exit 1
fi
