...
# e2e/suites/pavis/cases/19_http_version.sh
...
CASE_TMP=$(ensure_tmp_dir "pavis_19")
...
# 1. Create config with HTTP version specified
...
# 3. Test that connection works with H1
RESP=$(curl -s --http1.1 "http://127.0.0.1:$PORT_PAVIS/")
if [ "$RESP" != "backend-v1" ]; then
    echo "ERROR: Expected backend-v1, got $RESP"
    exit 1
fi
