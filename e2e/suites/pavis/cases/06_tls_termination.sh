#!/bin/bash
set -e

# e2e/suites/pavis/cases/06_tls_termination.sh

source "$E2E_ROOT/scripts/lib/process.sh"
source "$E2E_ROOT/scripts/lib/http.sh"
source "$E2E_ROOT/scripts/lib/fs.sh"

COMPOSE_FILE="$E2E_ROOT/config/pavis/docker-compose-pavis.yaml"

# Start backend services via docker-compose
docker-compose -f "$COMPOSE_FILE" up -d backend-v1 backend-v2 2>/dev/null || true
sleep 2

PORT_BACKEND=8081
PORT_PAVIS_TLS=8443

CASE_TMP=$(ensure_tmp_dir "pavis_06")

cleanup() {
    stop_pid "$CASE_TMP/pavis.pid"
    rm -f "$CASE_TMP/cert.pem" "$CASE_TMP/key.pem"
    docker-compose -f "$COMPOSE_FILE" down -v --remove-orphans 2>/dev/null || true
}
trap cleanup EXIT

# 1. Generate self-signed certificate
openssl req -x509 -newkey rsa:2048 -nodes \
    -keyout "$CASE_TMP/key.pem" \
    -out "$CASE_TMP/cert.pem" \
    -subj "/CN=localhost" \
    -days 1 2>/dev/null

# 2. Create TLS config
CONFIG_YAML="$CASE_TMP/config.yaml"
cat <<EOFCONFIG > "$CONFIG_YAML"
listeners:
  - name: "default"
    address: "127.0.0.1:$PORT_PAVIS_TLS"
    tls:
      cert_path: "$CASE_TMP/cert.pem"
      key_path: "$CASE_TMP/key.pem"
telemetry: {}
upstreams:
  - name: "backend"
    endpoints:
      - ip: "127.0.0.1"
        port: $PORT_BACKEND
routes:
  - host: "*"
    paths:
      - matcher: !prefix
          path: "/"
        destinations:
          - upstream: "backend"
            weight: 1
EOFCONFIG

CONFIG_PVS="$CASE_TMP/config.pvs"
"$PAVCTL_BIN" gen "$CONFIG_YAML" "$CONFIG_PVS"

# 3. Start Pavis
"$PAVIS_BIN" --config "$CONFIG_PVS" > "$CASE_TMP/pavis.log" 2>&1 &
echo $! > "$CASE_TMP/pavis.pid"
sleep 2

# 4. Test HTTPS connection (with insecure flag for self-signed cert)
RESPONSE=$(curl -k -s "https://127.0.0.1:$PORT_PAVIS_TLS/")
if [[ "$RESPONSE" != *"backend"* ]]; then
    echo "ERROR: Expected response from backend, got: $RESPONSE"
    exit 1
fi

echo "✅ Case 06_tls_termination passed"
