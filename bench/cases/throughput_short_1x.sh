...
run_wrk() {
  ...
  wrk -t "$THREADS" -c "$CONNECTIONS" -d "${duration}s" "${header_args[@]}" "$PROXY_URL" | tee "$outfile"
}
...
main() {
  ...
  for i in {1..30}; do
    if http_get "$BACKEND_URL" >/dev/null 2>&1; then
      echo "backend ready"
      break
    fi
    if [ "$i" -eq 30 ]; then
      echo "error: backend failed to become healthy after 30s" >&2
      exit 1
    fi
    sleep 1
  done
