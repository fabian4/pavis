...
# Case: throughput_short_1x
# Assumptions:
# - docker and docker compose are available.
# - wrk is installed on the host.
...
# When BENCH_LOADGEN_CPUSET is set to a single core, use 1 thread to respect the binding
# Otherwise, use 4 threads for maximum throughput
if [[ -n "${BENCH_LOADGEN_CPUSET:-}" ]]; then
  # Count CPUs in loadgen cpuset
  loadgen_cpu_count=$(echo "$BENCH_LOADGEN_CPUSET" | tr ',' '\n' | wc -l | tr -d ' ')
  THREADS="$loadgen_cpu_count"
else
  THREADS=4
fi
...
run_wrk() {
  ...
  local outfile="$2"
  local header_args=()
  if [ "$CHURN_CLOSE" -eq 1 ]; then
    header_args=(-H "Connection: close")
  fi
  local taskset_cmd=()
  ...
  if [ "${BENCH_VERBOSE:-0}" = "1" ]; then
    "${taskset_cmd[@]}" wrk -t "$THREADS" -c "$CONNECTIONS" -d "${duration}s" "${header_args[@]}" "$PROXY_URL" | tee "$outfile"
  else
    "${taskset_cmd[@]}" wrk -t "$THREADS" -c "$CONNECTIONS" -d "${duration}s" "${header_args[@]}" "$PROXY_URL" > "$outfile"
  fi
}
...
main() {
  ...
  # Only require wrk if not in dry-run mode
  ...
  if [ "${BENCH_VERBOSE:-0}" = "0" ]; then
    ...
    for run_dir in "${BASE_DIR}"/run_*; do
      if [ -f "${run_dir}/wrk.txt" ]; then
        ...
        local p99
        local errors
        rps=$(parse_rps "${run_dir}/wrk.txt")
        p99=$(to_ms "$(parse_latency_pct "99%" "${run_dir}/wrk.txt")")
        errors=$(parse_errors "${run_dir}/wrk.txt")
  ...
  end_ts=$(date +%s)
  bench_print_duration $((end_ts - start_ts))
  # Raw outputs kept: run_*/ subdirectories for summarize_github.sh to aggregate
}
