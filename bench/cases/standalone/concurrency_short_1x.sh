...
CONNECTIONS="${STANDALONE_CONCURRENCY_SHORT_1X_CONNECTIONS}"
TARGET_RPS="${STANDALONE_CONCURRENCY_SHORT_1X_TARGET_RPS}"
REQUEST_PATH="/fixed"
...
run_wrk() {
  ...
  if [ "${BENCH_VERBOSE:-0}" = "1" ]; then
    "${taskset_cmd[@]}" wrk -t "$THREADS" -c "$CONNECTIONS" -d "${duration}s" "$PROXY_URL" | tee "$outfile"
  else
    "${taskset_cmd[@]}" wrk -t "$THREADS" -c "$CONNECTIONS" -d "${duration}s" "$PROXY_URL" > "$outfile"
  fi
}
