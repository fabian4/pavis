...
DURATION_S="${STANDALONE_LATENCY_EXTENDED_1X_DURATION_S}"
WARMUP_S=5
COOLDOWN_S=5
CONNECTIONS="${STANDALONE_LATENCY_EXTENDED_1X_CONNECTIONS}"
TARGET_RPS="${STANDALONE_LATENCY_EXTENDED_1X_TARGET_RPS}"
RUN_COUNT=5
...
main() {
  ...
  end_ts=$(date +%s)
  bench_print_duration $((end_ts - start_ts))
  # Raw outputs kept in run_*/ subdirectories for summarize_github.sh to aggregate
}
