...
upstreams: [{ name: "backend-dns", discovery: logical, endpoints: [{ address: "localhost", port: 8081 }] }]
routes: [{ host: "*", paths: [{ matcher: !prefix { path: "/" }, destinations: [{ upstream: "backend-dns", weight: 1 }] }] }]
EOF
