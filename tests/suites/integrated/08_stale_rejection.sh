...
gen_pvs "$TEST_TMP/ingest.yaml" "$TEST_TMP/v10.pvs"
curl -s -X POST "http://127.0.0.1:$PORT_RELAY/v1/publish" -H "X-Pavis-Version: 10" --data-binary "@$TEST_TMP/v10.pvs"

run_pavis "$TEST_TMP/v10.pvs" "http://127.0.0.1:$PORT_RELAY"
...
rm -rf "$TEST_TMP/storage"; mkdir -p "$TEST_TMP/storage"

run_relay "$TEST_TMP/relay.yaml" "relay_reset"
wait_for_url "http://127.0.0.1:$PORT_RELAY/health" 5
...
echo "✅ Case 08_stale_rejection passed"
